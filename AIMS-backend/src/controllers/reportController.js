...
export async function getJobSankey(req, res) {
	try {
		...
		const sourceCompanyLinksAgg = await jobsCollection.aggregate([
			...
			{
				$group: {
					_id: { source: '$source', company: '$company' },
					count: { $sum: 1 }
				}
			}
		]).toArray();

		const companyStatusLinksAgg = await jobsCollection.aggregate([
			{ $match: { 'applied.0': { $exists: true } } },
			{ $unwind: '$applied' },
			{
				$project: {
		...
		sourceCompanyLinksAgg.forEach(link => {
			addNode(link._id.source);
			addNode(link._id.company);
		});
		companyStatusLinksAgg.forEach(link => {
			addNode(link._id.company);
			addNode(link._id.status);
		});

		const links = [];
		sourceCompanyLinksAgg.forEach(link => {
			if (nodeMap.has(link._id.source) && nodeMap.has(link._id.company)) {
				links.push({
					source: nodeMap.get(link._id.source),
					target: nodeMap.get(link._id.company),
					value: link.count
				});
			}
		});
		companyStatusLinksAgg.forEach(link => {
			if (nodeMap.has(link._id.company) && nodeMap.has(link._id.status)) {
				links.push({
					source: nodeMap.get(link._id.company),
					target: nodeMap.get(link._id.status),
					value: link.count
				});
			}
		});
