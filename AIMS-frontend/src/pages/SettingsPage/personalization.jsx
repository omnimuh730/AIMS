...
const PersonalizationPage = () => {
	...
	useEffect(() => {
		const fetchSkills = async () => {
			...
			try {
				...
				const res = await get(`http://localhost:3001/api/skills-category?${params.toString()}`);
	...
	const handleToggleSkill = async (skill) => {
		...
		try {
			...
			const res = await post('http://localhost:3001/api/personal/skills/update', { skills: nextSkills });
			if (res && res.success && Array.isArray(res.skills)) {
				setUserSkills(res.skills);
