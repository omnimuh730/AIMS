...
import {
  List, ListItemButton, Typography, Chip, Box, Grid
} from '@mui/material';
...
const MailPage = () => {
  ...
  const { labelTreeItems, labelMap } = useMemo(() => {
    const filteredUserLabels = labels.filter(label => label.type === 'user');
    ...
    return { userLabels: filteredUserLabels, labelTreeItems: toTreeItems(tree), labelMap };
  }, [labels]);
        ...
        <Chip
          ...
          sx={{
            ...
            ...(color && color.backgroundColor ? {
              ...
              '.MuiChip-label': { color: color.textColor || '#000' }
            } : {})
          }}
        />
      </TreeItemLabel>
  ...
  const CustomTreeItem = React.forwardRef(function CustomTreeItem(props, ref) {
    const { id, itemId, label, color, disabled, children, ...other } = props;
    const {
        ...
        <Grid size = {2}>
          ...
          <Box sx={{ bgcolor: '#fff', borderRadius: 2, p: 2, boxShadow: 1 }}>
            <RichTreeView
              ...
              defaultExpandedItems={labelTreeItems.map(item => item.id)}
        ...
        <Grid size = {10}>
          ...
          <List>
            {emails.map(email => (
              ...
              >
                <Typography fontWeight="bold" component="span" noWrap sx={{ minWidth: 180, maxWidth: 220, flexShrink: 0 }}>
                  {email.from}
                </Typography>
                {email.labelIds && email.labelIds.map(labelId => {
                  ...
                  return labelObj ? (
                    <Chip
                      ...
                      sx={{
                        maxWidth: 100,
                        mr: 0.5,
                        ...(labelObj.color && labelObj.color.backgroundColor ? { bgcolor: labelObj.color.backgroundColor, color: labelObj.color.textColor || '#000' } : {})
                      }}
                    />
                  ) : null;
                })}
                <Typography variant="subtitle1" fontWeight="bold" component="span" noWrap sx={{ flex: 1, minWidth: 0, mx: 1 }}>
                  {email.subject}
                </Typography>
                <Typography variant="body2" color="text.secondary" component="span" noWrap sx={{ flex: 2, minWidth: 0, mx: 1 }}>
                  {email.body?.replace(/<[^>]+>/g, '').slice(0, 80) || email.snippet}
                </Typography>
                <Typography variant="caption" color="text.secondary" component="span" noWrap sx={{ minWidth: 120, textAlign: 'right', flexShrink: 0 }}>
